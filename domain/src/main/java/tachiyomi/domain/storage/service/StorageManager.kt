...
class StorageManager(
    ...
    fun getDownloadsDirectory(): UniFile? =
        baseDir?.createDirectory(DOWNLOADS_PATH)

    fun getLocalSourceDirectory(): UniFile? =
        baseDir?.createDirectory(LOCAL_SOURCE_PATH)

    fun getLogsDirectory(): UniFile? =
        baseDir?.createDirectory(LOGS_PATH)

    companion object {

        fun directoryAccessible(context: Context, uri: String): Boolean {
            return UniFile.fromUri(context, uri.toUri())?.exists() == true
        }
